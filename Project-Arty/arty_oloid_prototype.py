# ==============================================================================
# VERSION: v5.3.2-Logged-Experimental
...
class NKSTParametricEngine:
    ...
    def process_token_step(self, step_idx: int, word: str) -> Dict[str, Any]:
        ...
        payload = {
            ...
            "action": action,
            "temporal": self.compute_temporal_distance()
        }
        self.processed_tokens_history.append(payload)
        return payload

    def compute_temporal_distance(self) -> dict:
        """
        NKST FRAMEWORK USE: Fifth pillar — temporal distance metric.
        Measures distance from boundary and vacuum in units of 1/(6φ³),
        the natural decay unit observed as a structural invariant in
        GIO v0.2 stability testing.
        """
        phi_sixth_unit = 1.0 / (6.0 * (self.PHI ** 3))
    
        distance_to_horizon = (self.HORIZON_LIMIT - self.system_load) / phi_sixth_unit
        distance_to_vacuum = (self.system_load - self.VACUUM_GROUND) / phi_sixth_unit
        steps_per_correction = round(self.GOLDEN_HARDDRIVE_DECAY / phi_sixth_unit)
    
        return {
            "phi_sixth_unit": phi_sixth_unit,
            "distance_to_horizon": distance_to_horizon,
            "distance_to_vacuum": distance_to_vacuum,
            "steps_per_correction": steps_per_correction
        }
    ...
    def run_generation_loop(self, max_steps: int = 12, export_filename: str = "nkst_session_log.json"):
        ...
        for step in range(max_steps):
            ...
            print(f"Step #{res['step']:02d} | Generated: '{res['word']:<32}' | "
                  ...
                  f"Axis: {res['axis']:<18} | "
                  f"Temp: {res['temp']:.3f}\n ↳ Action: {res['action']}")
            print(f" ↳ Temporal Distance to Horizon: {res['temporal']['distance_to_horizon']:.3f} units | "
                  f"Steps/Correction: {res['temporal']['steps_per_correction']} | "
                  f"Unit: 1/(6φ³) ≈ {res['temporal']['phi_sixth_unit']:.6f}")            
            print("-" * 76)
